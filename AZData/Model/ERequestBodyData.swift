...
public class ERequestBodyData: NSManagedObject, Entity {
    ...
    public func copyEntity(_ toProj: EProject, ctx: NSManagedObjectContext) -> ERequestBodyData? {
        let id = Self.db.requestBodyDataId()
        let wsId = toProj.getWsId()
        let body = Self.db.createRequestBodyData(id: id, wsId: wsId, ctx: ctx)
        body?.json = self.json
        body?.raw = self.raw
        ...
        if let xs = self.multipart?.allObjects as? [ERequestData] {
            xs.forEach { multipart in
                let newMultipart = multipart.copyEntity(wsId: wsId, ctx: ctx)
