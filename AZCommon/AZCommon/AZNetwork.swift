...
public enum AZReachabilityError: Error {
...
public final class AZReachability {
    public static let shared = AZReachability()
    private let defaultQueue = DispatchQueue(label: "net.jsloop.az.default-queue", qos: .default, attributes: .concurrent, autoreleaseFrequency: .workItem, target: nil)
    ...
    public var whenReachable: ((AZReachability) -> Void)?
    public var whenUnreachable: ((AZReachability) -> Void)?
    ...
    required public init(reachabilityRef: SCNetworkReachability) {
        ...
        self.serialQueue = self.defaultQueue
    }
    
    public convenience init(hostname: String) throws {
        guard let ref = SCNetworkReachabilityCreateWithName(nil, hostname) else { throw AZReachabilityError.failedToCreateWithHostname(hostname, SCError()) }
        self.init(reachabilityRef: ref)
    }
    
    public convenience init() {
        self.init(reachabilityRef: AZReachability.getReachabilityForZeroAddress()!)
    }
    ...
    func setReachabilityFlags() throws {
        try self.serialQueue.sync { [unowned self] in
            ...
            if !SCNetworkReachabilityGetFlags(self.reachabilityRef, &flags) {
                self.stopNotifier()
                throw AZReachabilityError.unableToGetFlags(SCError())
            }
            self.flags = flags
        }
    }
    ...
    public func startNotifier() throws {
        ...
        let callback: SCNetworkReachabilityCallBack = { reachability, flags, info in
            ...
            let weakReachability = Unmanaged<AZReachabilityBridge>.fromOpaque(info).takeUnretainedValue()
        ...
        let weakReachability = AZReachabilityBridge(reachability: self)
        let opaqueWeakReachability = Unmanaged<AZReachabilityBridge>.passUnretained(weakReachability).toOpaque()
        var context = SCNetworkReachabilityContext(
            version: 0,
            info: UnsafeMutableRawPointer(opaqueWeakReachability),
            retain: { (info: UnsafeRawPointer) -> UnsafeRawPointer in
                let unmanagedWeakReachability = Unmanaged<AZReachabilityBridge>.fromOpaque(info)
        ...
        if !SCNetworkReachabilitySetCallback(self.reachabilityRef, callback, &context) {
            self.stopNotifier()
            throw AZReachabilityError.unableToSetCallback(SCError())
        }
        if !SCNetworkReachabilitySetDispatchQueue(reachabilityRef, self.serialQueue) {
            self.stopNotifier()
            throw AZReachabilityError.unableToSetDispatchQueue(SCError())
        }
...
extension SCNetworkReachabilityFlags {
    typealias Connection = AZReachability.Connection
...
private final class AZReachabilityBridge {
    weak var reachability: AZReachability?
    init(reachability: AZReachability) {
...
public protocol AZHTTPClientDelegate: AnyObject {
...
public final class AZHTTPClient: NSObject {
    private lazy var queue: OperationQueue = {
        let q = OperationQueue()
        q.name = "net.jsloop.az.httpclient"
    ...
    public weak var delegate: AZHTTPClientDelegate?
...
extension AZHTTPClient: URLSessionDelegate {
...
extension AZHTTPClient: URLSessionDataDelegate {
